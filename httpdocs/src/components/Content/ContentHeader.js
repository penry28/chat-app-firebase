...
import { SET_USERS_IN_CURRENT_ROOM, SET_IS_SHOW_POPUP_ADD_INVITE } from '../../store/types';
              ...
              ${usersFiteredInRoom && usersFiteredInRoom.map(user => `
                <li data-user="${JSON.stringify(user)}">
                  ${
                    ...
                    : `<div title="${auth.user.uid === user.uid ? 'Bạn' : user.displayName}" style="width: 30px; height: 30px; border-radius: 50%; border: 1px solid #ccc; margin-right: 4px; background-color: #ccc"></div>`
                  }
                </li>
              `)}
            </ul>
