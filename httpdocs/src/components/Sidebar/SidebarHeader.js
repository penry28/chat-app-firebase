...
const SidebarHeader = ({ auth: { user } }) => {
  return html`
    <div class="main-sidebar__header d-flex justify-content-between align-items-center">
      <div class="main-sidebar__user-info d-flex align-items-center">
        ${
          user.photoURL
          ? `<img class="avatar" src="${user.photoURL}" alt="" />`
          : `<span style="width: 50px; height: 50px; background-color: #ccc; border-radius: 50%;" class="avatar d-flex align-items-center justify-content-center">${user?.displayName.charAt(0)}</span>`
        }
        <span class="username ml-2">${user.displayName}</span>
      </div>
