...
import { USER_DB_NAME, ROOM_DB_NAME, MESSAGE_DB_NAME } from '../firebase/constants';
...
export default function reducer(state = init, action, args) {
  switch (action) {
    ...
    case SUBMIT_SEND_MESSAGE:
      const messageSendText = args;
      const { currentRoom } = state.chat;
      const { user } = state.auth;
      if (!!messageSendText.length) {
          const addMessage = async(message, roomId, userId) => {
              try {
                  await addDoc(collection(db, MESSAGE_DB_NAME), {
                      message,
                      roomId,
                      userId,
                      createdAt: serverTimestamp()
          ...
          addMessage(messageSendText, currentRoom, user.uid)
              .then(() => {
                  dispatch(SET_MESSAGE_SEND, '');
      ...
      return state;
    case SET_MESSAGE_IN_CURRENT_ROOM:
      return {
        ...state,
        chat: {
          ...state.chat,
    ...
    case DELETE_USER_IN_ROOM:
      if (confirm('Bạn chắc chắn muốn xóa user này?')) {
        const usersInRoom = state.chat.usersInRoom.map(item => item.uid);
        (async (users, userDelete) => {
          const roomRef = doc(db, ROOM_DB_NAME, state.chat.currentRoom);
          const usersUpdate = users.filter(item => item !== userDelete);
          await updateDoc(roomRef, {
            users: usersUpdate
          });
        })(usersInRoom, args);
